...
class Pickets:
    ...
    def __create_pickets(self) -> NoReturn:
        ...
        for i, pnt in enumerate(self.__line):
            if i == len(self.__line) - 1:
                return
            if i == 0:
                self.__pickets.append(Pickets(pnt, f'{self.__txt}'))
            ...
            if vector.length() == self.__distance - self.__domer:
                self.__domer = 0.0
                self.__pickets.append(Pickets(pnt, f'{self.__txt}{namepk}'))
                namepk += 1
                continue
            ...
            if vector.length() > self.__distance - self.__domer:
                ...
                while l <= vector.length():
                    ...
                    if self.__pickets[-1].point != p:
                        self.__pickets.append(Pickets(p, f'{self.__txt}{namepk}'))
                        namepk += 1
                    l += self.__distance
                    self.__domer = 0.0
